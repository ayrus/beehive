...
func TestSimpleLPM(t *testing.T) {
	...
	res, err := kv.Process(context.Background(), lpm.Get(net.ParseIP("1.1.1.1")))
	if err == nil {
...
func TestMiss1(t *testing.T) {
	...
	kv.Process(context.Background(), lpm.Put(r))
	res, err := kv.Process(context.Background(), lpm.Get(net.ParseIP("4.4.4.4")))
	if err == nil {
		if err == nil {
			if res != nil {
...
func TestMiss2(t *testing.T) {
	...
	kv.Process(context.Background(), lpm.Put(r))
	res, err := kv.Process(context.Background(), lpm.Get(net.ParseIP("255.0.0.0")))
	if err == nil {
		if err == nil {
			if res != nil {
				t.Error("Found a result when there should not have been one: ", res)
...
func TestHighPriority1(t *testing.T) {
	...
	res, err := kv.Process(context.Background(), lpm.Get(net.ParseIP("123.123.123.123")))
	if err == nil {
...
func TestPrefixMatchLong(t *testing.T) {
	...
	res, err := kv.Process(context.Background(), lpm.Get(net.ParseIP("255.255.255.255")))
	if err == nil {
		if res == nil || !(compareRoute(r1, res.(lpm.Route))) {
			t.Error("Returned the wrong result: ", res)
		}
